...
function Borrow({ onClickRow }) {
	...
	const confirm = (rowData) => {
		setState(oldState => ({ ...oldState, showBar: true, clickedTableRow: rowData }))
	...
	return (
		<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
			<div className={styles.buttonConteiner}>
				...
				<Button onClick={sendToApi} variant="contained" size="medium" color="primary" className={styles.button}>Calcular</Button>
			</div>
			<TableContainer component={Paper} className={styles.table}>
				<Table>
					<TableHead>
						<TableRow>
							<TableCell
								style={{ color: "#228A95", background: '#F8F8F8', fontWeight: "bold", fontSize: 24, }}
								align="center"
								colSpan={5}
							>
								Tabela Padrão
						...
						<TableRow style={{ background: '#F8F8F8', fontWeight: "bold", fontSize: 24, }}>
							...
							<TableCell className={styles.rowHead}>Comissão Parceiro</TableCell>
						</TableRow>
					</TableHead>
					<TableBody>
						{tableData.map(data => (
